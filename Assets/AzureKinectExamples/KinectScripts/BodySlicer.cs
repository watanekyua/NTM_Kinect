    ...
    public class BodySlicer : MonoBehaviour
    {
        ...
        public bool EstimateBodySlices(ulong userId)
        {
                ...
                sensorData.lastDepthFrameTime != lastDepthFrameTime)
            {
                ...
                {
                    bodySlices[(int)BodySlice.HEIGHT] = GetUserHeightParams(pointPelvis);
                }

                if (estimateBodyWidth)
                {
                    bodySlices[(int)BodySlice.WIDTH] = GetUserWidthParams(pointPelvis);
                }
                ...
                if (estimateBodySlices && kinectManager.IsJointTracked(userId, (int)KinectInterop.JointType.Pelvis) && kinectManager.IsJointTracked(userId, (int)KinectInterop.JointType.Neck))
                {
                    ...
                    bodySlices[(int)BodySlice.TORSO_1] = GetBodySliceParams(BodySlice.TORSO_1, vSlicePoint, true, false, -1);

                    vSlicePoint += sliceDir;
                    bodySlices[(int)BodySlice.TORSO_2] = GetBodySliceParams(BodySlice.TORSO_2, vSlicePoint, true, false, -1);

                    vSlicePoint += sliceDir;
                    bodySlices[(int)BodySlice.TORSO_3] = GetBodySliceParams(BodySlice.TORSO_3, vSlicePoint, true, false, -1);

                    vSlicePoint += sliceDir;
                    bodySlices[(int)BodySlice.TORSO_4] = GetBodySliceParams(BodySlice.TORSO_4, vSlicePoint, true, false, -1);
                }
            }
