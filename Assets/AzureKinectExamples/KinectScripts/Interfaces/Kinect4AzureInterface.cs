...
namespace com.rfilkov.kinect
{
    ...
    public class Kinect4AzureInterface : DepthSensorBase
    {
        ...
        public override KinectInterop.SensorData OpenSensor(KinectManager kinectManager, KinectInterop.FrameSource dwFlags, bool bSyncDepthAndColor, bool bSyncBodyAndDepth)
        {
            ...
            if (consoleLogMessages)
                Debug.Log("Kinect4Azure-sensor opened: " + sensorDeviceId);
        ...
        public override void CloseSensor(KinectInterop.SensorData sensorData)
        {
            ...
            if (consoleLogMessages)
                Debug.Log("Kinect4Azure-sensor closed: " + sensorDeviceId);
        }
        ...
        public override bool UpdateSensorData(KinectInterop.SensorData sensorData, KinectManager kinectManager, bool isPlayMode)
        {
            ...
            if (detectFloorForPoseEstimation && floorDetector != null)
            {
                if(floorDetector.UpdateFloorDetector(rawDepthImage, rawDepthTimestamp, ref depthFrameLock, minDistance, maxDistance))
                {
                    lock (poseFrameLock)
                    {
        ...
        public override void PollSensorFrames(KinectInterop.SensorData sensorData)
        {
            try
            {
                if (kinectPlayback != null)
                {
                    ...
                    long currentPlayTime = isSensorMaster || syncSensorIndex < 0 ? 
                    ...
                    if ((frameSourceFlags & (KinectInterop.FrameSource)0x7F) != 0)
                    {
                        ...
                        //Debug.Log("D" + deviceIndex + " PlayTime: " + currentPlayTime + ", capture: " + capture);
                    ...
                    if (isCamerasStarted)
                    {
                        ...
                        if (bufCaptureLen > 0)
                        {
                            if (sensorSyncher != null && syncSensorIndex >= 0)  // check for synched device
                            {
                                ...
                                {
                                    //Debug.Log("D" + deviceIndex + " Got capture. Timestamp: " + bufCapTimes[ci] + ", CapIndex: " + ci + ", Now: " + DateTime.Now.ToString("HH:mm:ss.fff"));
        ...
        private void ProcessCameraFrames(KinectInterop.SensorData sensorData, Capture capture)
        {
            ...
            if (bodyTracker != null)
            {
                ...
                {
                    //Debug.Log("D" + deviceIndex + " Syncher-UpdatePushBtCapture. Timestamp: " + capture.Depth.DeviceTimestamp.Ticks);
                    sensorSyncher.UpdatePushBtCapture(syncSensorIndex, capture.Depth.DeviceTimestamp.Ticks, capture);
                    return;
                }
            ...
            if (capture != null && (!isSyncBodyAndDepth || btCapture != null))
            {
                ...
                if (sensorSyncher != null && syncSensorIndex >= 0)
                {
                    ...
                    sensorSyncher.UpdateCapture(syncSensorIndex, capTimestamp, capture);
                    return;
                }
        ...
        public void ProcessSensorCapture(KinectInterop.SensorData sensorData, Capture capture)
        {
            ...
            lock (sensorCaptureLock)
            {
                ...
                if ((pointCloudAlignedColorTex != null || pointCloudDepthBuffer != null || sensorData.colorDepthBuffer != null || sensorData.colorCamDepthImage != null) &&
                ...
                while (bodyFrameHandle != IntPtr.Zero)
                {
                    ...
                    if(bodyFrameHandleNext == IntPtr.Zero || (sensorSyncher != null && syncSensorIndex >= 0))
                    {
                        ...
                        if (sensorSyncher != null && syncSensorIndex >= 0)
                        {
                            ...
                            sensorSyncher.UpdateBtFrame(syncSensorIndex, btTimestamp);
                        }
                    }
                    else
                    {
                    ...
                    if (processFrameData)
                    {
                        ...
                        if (/**rawBodyIndexImage != null &&*/ (getAllSensorFrames || rawBodyIndexTimestamp == sensorData.lastBodyIndexFrameTime))
                        {
                                ...
                                (!isSyncBodyAndDepth || (rawBodyIndexTimestamp == depthFrameTime)) && (rawBodyIndexTimestamp != sensorData.lastBodyIndexFrameTime))
                            {
                                ...
                                {
                                    coordMapperTransform.DepthImageToColorCameraCustom(btCapture.Depth, bodyIndexImage, c2DepthImage, c2BodyIndexImage, TransformationInterpolationType.Linear, 255);
        ...
        private void CalcBodyJointOrients(ref KinectInterop.BodyData bodyData)
        {
