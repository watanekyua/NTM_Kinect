...
namespace com.rfilkov.kinect
{
    ...
    public class KinectManager : MonoBehaviour
    {
        ...
        {
            return GetJointPosColorOverlay(userId, joint, sensorIndex, camera, imageRect);
        }
        ...
        private DepthSensorBase CreateSensorInterface(KinectInterop.DepthSensorPlatform sensorType, int deviceIndex)
        {
            ...
            switch (sensorType)
            {
                ...
                default:
                    throw new Exception("Cannot create sensor interface of type: " + sensorType);
            }
        ...
        private DepthSensorBase.BaseSensorSettings GetSensorSettings(KinectInterop.DepthSensorPlatform sensorType, string sJsonSettings)
        {
                ...
                if (useMultiCamConfig)
                {
                    ...
                    if (!string.IsNullOrEmpty(multiCamJson))
                    {
                        ...
                        for (int i = 0; i < multiCamConfig.camPose.Length; i++)
                        {
                            ...
                            {
                                // set settings
                                DepthSensorBase.BaseSensorSettings settings = GetSensorSettings(sensorType, multiCamConfig.settings[i]);
                                if (settings != null)
                                {
        ...
        protected void UpdateTrackedBodies(int sensorIndex, KinectInterop.SensorData sensorData, ulong prevBodyFrameTime)
        {
            ...
            {
                btSensorIndex = sensorIndex;
                List<KinectInterop.BodyData> alMergedBodies = userBodyMerger.MergeUserBodies(ref lastBodyFrameTime);
